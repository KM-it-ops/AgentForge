#!/usr/bin/env node
// Sync the AUTO-LOCAL-SKILLS section of ~/.claude/CLAUDE.md from skills/*/SKILL.md.
// - Skips _archived/
// - Parses YAML frontmatter for name + description
// - Compresses description into a single-line trigger string
// - Rewrites only the content between marker comments
//
// Triggered by:
//   PostToolUse (Write|Edit) when a SKILL.md is created/edited
//   SessionStart (catches skills added outside Claude Code)
//
...
function parseFrontmatter(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const key = kv[1].trim();
    let val = kv[2].trim();
    // Handle YAML block scalars: `key: >` or `key: |` followed by indented lines
    if (val === ">" || val === "|") {
      const collected = [];
      let j = i + 1;
...
function condense(desc, maxLen = 110) {
  ...
  // first sentence, collapse whitespace, trim
...
function listLocalSkills() {
  ...
  return fs
    ...
    .filter((d) => {
      if (d.name === "_archived" || d.name.startsWith(".")) return false;
...
function readSkill(name) {
  ...
  return { name, description: fm?.description || "(no description in frontmatter)" };
}

function buildBlock(skills) {
  const rows = skills
    .map((s) => `| \`${s.name}\` | ${condense(s.description)} |`)
    .join("\n");
  return [
...
function main() {
  ...
  if (startIdx === -1 || endIdx === -1) {
    console.error("AUTO-LOCAL-SKILLS markers missing in CLAUDE.md; skipping sync.");
    process.exit(0);
  }
  ...
  if (next !== md) {
    ...
    console.log(`sync-local-skill-router: updated ${skills.length} local skill rows`);
  }
}
